  ...
  size_t current_alignment)
{
  ...
  if (is_plain) {
    ...
    using DataType = crazyflie_interfaces::srv::AddLogging_Request;
    is_plain =
      (
...
static size_t _AddLogging_Request__max_serialized_size(char & bounds_info)
{
  ...
  ret_val = max_serialized_size_AddLogging_Request(full_bounded, is_plain, 0);
  ...
  size_t current_alignment)
{
  ...
  if (is_plain) {
    ...
    // We still need to check that the in-memory alignment
    // is the same as the CDR mandated alignment.
    using DataType = crazyflie_interfaces::srv::AddLogging_Response;
    is_plain =
      (
...
static size_t _AddLogging_Response__max_serialized_size(char & bounds_info)
{
  ...
  ret_val = max_serialized_size_AddLogging_Response(full_bounded, is_plain, 0);
