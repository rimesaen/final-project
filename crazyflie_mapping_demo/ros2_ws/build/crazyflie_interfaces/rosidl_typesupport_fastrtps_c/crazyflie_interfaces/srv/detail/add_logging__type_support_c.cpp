  ...
  size_t current_alignment)
{
  ...
  if (is_plain) {
    ...
    using DataType = crazyflie_interfaces__srv__AddLogging_Request;
    is_plain =
      (
      offsetof(DataType, vars) +
      last_member_size
      ) == ret_val;
  }
...
static size_t _AddLogging_Request__max_serialized_size(char & bounds_info)
{
  bool full_bounded;
  bool is_plain;
  size_t ret_val;

  ret_val = max_serialized_size_crazyflie_interfaces__srv__AddLogging_Request(
  ...
  size_t current_alignment)
{
  ...
  if (is_plain) {
    ...
    // We still need to check that the in-memory alignment
    // is the same as the CDR mandated alignment.
    using DataType = crazyflie_interfaces__srv__AddLogging_Response;
    is_plain =
      (
...
static size_t _AddLogging_Response__max_serialized_size(char & bounds_info)
{
  bool full_bounded;
  bool is_plain;
  size_t ret_val;

  ret_val = max_serialized_size_crazyflie_interfaces__srv__AddLogging_Response(
    full_bounded, is_plain, 0);
